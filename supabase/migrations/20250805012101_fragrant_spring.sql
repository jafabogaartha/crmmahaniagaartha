...
-- RLS Policies for users table (fixed to avoid recursion)
-- Allow public access for login (no auth.uid() check to avoid recursion)
CREATE POLICY "Public can read users for login" ON users 
  FOR SELECT 
  TO public 
  USING (true);
...
-- Allow superadmin to manage all users (only for non-SELECT operations)
...
CREATE POLICY "Superadmin can manage products" ON products 
  FOR ALL 
  TO authenticated 
  USING (
    EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'superadmin')
...
CREATE POLICY "All authenticated users can read packages" ON packages 
  FOR SELECT 
  TO authenticated 
  USING (true);

CREATE POLICY "Superadmin can manage packages" ON packages 
  FOR ALL 
  TO authenticated 
  USING (
    EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'superadmin')
...
CREATE POLICY "Users can read leads based on role" ON leads 
  FOR SELECT 
  TO authenticated 
  USING (
    EXISTS (
...
CREATE POLICY "Admins can update their leads" ON leads 
  FOR UPDATE 
  TO authenticated 
  USING (
    EXISTS (
...
CREATE POLICY "Users can read notes for accessible leads" ON notes 
  FOR SELECT 
  TO authenticated 
  USING (
    EXISTS (
...
CREATE POLICY "Users can insert notes for accessible leads" ON notes 
  FOR INSERT 
  TO authenticated 
  WITH CHECK (
    EXISTS (
...
CREATE POLICY "All authenticated users can read targets" ON targets 
  FOR SELECT 
  TO authenticated 
  USING (true);

CREATE POLICY "Superadmin can manage targets" ON targets 
  FOR ALL 
  TO authenticated 
  USING (
    EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'superadmin')
...
CREATE POLICY "HC and superadmin can access handle customer data" ON handle_customer_data 
  FOR ALL 
  TO authenticated 
  USING (
    EXISTS (
