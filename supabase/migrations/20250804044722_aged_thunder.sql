/*
  ...
  1. New Tables
    ...
    - `notes` - Lead notes and communication history
    - `targets` - Admin performance targets
  ...
  3. Features
    - Automatic lead assignment with round-robin
    - Lead stage tracking and conversion analytics
...
CREATE POLICY "Superadmin can manage products" ON products FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'superadmin')
);

-- RLS Policies for packages table
CREATE POLICY "All authenticated users can read packages" ON packages FOR SELECT TO authenticated USING (true);
CREATE POLICY "Superadmin can manage packages" ON packages FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'superadmin')
);

-- RLS Policies for leads table
CREATE POLICY "Users can read leads based on role" ON leads FOR SELECT TO authenticated USING (
  EXISTS (
    SELECT 1 FROM users 
    WHERE id = auth.uid() 
    AND (
...
CREATE POLICY "Admins can update their leads" ON leads FOR UPDATE TO authenticated USING (
  EXISTS (
    SELECT 1 FROM users 
    WHERE id = auth.uid() 
    AND (
...
CREATE POLICY "System can insert leads" ON leads FOR INSERT TO authenticated USING (true);

-- RLS Policies for notes table
CREATE POLICY "Users can read notes for accessible leads" ON notes FOR SELECT TO authenticated USING (
  EXISTS (
...
CREATE POLICY "Users can insert notes for accessible leads" ON notes FOR INSERT TO authenticated USING (
  EXISTS (
    SELECT 1 FROM leads l
    JOIN users u ON u.id = auth.uid()
    WHERE l.id = notes.lead_id
...
CREATE POLICY "All authenticated users can read targets" ON targets FOR SELECT TO authenticated USING (true);
CREATE POLICY "Superadmin can manage targets" ON targets FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'superadmin')
);

-- RLS Policies for handle_customer_data table
CREATE POLICY "HC and superadmin can access handle customer data" ON handle_customer_data FOR ALL TO authenticated USING (
  EXISTS (
    SELECT 1 FROM users 
    WHERE id = auth.uid() 
